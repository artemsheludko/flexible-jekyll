...
the addition of CAN termination resistors as convenience for so that they are
...
The testing I completed for each board can be broken up into 4 categories; power
...
converter output was 5 V, which was successfully converted to 3.3 V by the LDO
...
board which succeeded. The debug test was carried out using the `gdb-multiarch`
...
as expecte using voltage probes without writing any software. As discussed in
the earlier current sensor section, the zero current voltage output is 2.5 V.
Probing, the sensor output with any load connected read as 2.5 V on the DMM and
...
to the ME over both CAN controller which were working. Further work will be done
